...
class Lead extends MY_Controller {
	...
	function __construct()
	{
		...
		$this->m_konfig->validasi_session(array("leader"));
		
		date_default_timezone_set("Asia/Jakarta");
	}
	 function updateTarget()
	 {
		 $updateTarget=$this->event->updateTarget();
		 echo json_encode($updateTarget);
	 }
	 function alokasi()
	{
	...
	}function detailBlok()
	{
 		...
 		$data["d"] = $d;
		echo $this->load->view("detailBlok", $data);
	}function getDetail()
	{
		if($this->input->post("id")==1)
		{
			$this->load->view('getDetailUpacara');
		}elseif($this->input->post("id")==4)
		{
			$this->load->view('getDetailSuci');
		}else{
	...
	}public function hutri()
	{
		 
		$ajax=$this->input->get_post("ajax");
		if($ajax=="yes")
		{
			echo	$this->load->view("hutri");
		}else{
			$data['konten']="hutri";
			$this->_template($data);
		}
