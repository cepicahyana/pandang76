    ...
    <div class="modal-content">
      ...
      <div class="box box-primary">
       <div class="box-header">
          <h5 class="box-title ">Silahkan <a href='<?php echo base_url();?>FORMAT2.xlsx'>
          ...
          <div class="box-body">                      
            <form role="form" name="uploadfilexl" id="uploadfilexl" 
				...
				<br>
                <button type="submit" onclick="javascript:simpanfile();" class="btn pull-right bg-teal">
                ...
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                    <div class="card" >
                        <div class="header"> 
                         ...
                         <button    onclick="import_file_khusus(<?php echo $id; ?>)" class="btn bg-teal waves-effect"><i class="material-icons">file_download</i>IMPORT FORMAT </button>
...
$dbasex[2]="ACARA SORE";
$array=$dbasex;
 echo form_dropdown("waktu",$array,"",'class="form-control input-sm" id="waktu" onchange="reload_table()"');
 ?>
 </div>
...
$dbasex[""]="==== KEHADIRAN ====";
$dbasex[2]="HADIR";
$dbasex[1]="TIDAK/BELUM HADIR";
$dbasex[0]="DICEKAL";
$array=$dbasex;
 echo form_dropdown("status",$array,"",'class="form-control input-sm" id="status" onchange="reload_table()"');
 ?>
 </div>
 ...
 $dbas="";
$dbas["0"]="==== LEMBAGA ====";
$dblok=$this->db->query("select distinct(lembaga) as lembaga from data_peserta")->result();
foreach($dblok as $blok)
{
	$dbas[$blok->lembaga]=$blok->lembaga;
}
$dbas=$dbas;
 echo form_dropdown("lembaga",$dbas,"0",'class="form-control input-sm" id="lembaga" onchange="reload_table()"');
 ?>
 </div>
...
$dbas["2"]=" SELAIN CADANGAN ";
$dbas["0"]="==== ALL ====";
$dbas["1"]=" CADANGAN SAJA ";
$dbas=$dbas;
 echo form_dropdown("cadangan",$dbas,"0",'class="form-control input-sm" id="cadangan" onchange="reload_table()"');
 ?>
 </div>
 ...
 $dbas="";
$dbas["0"]="==== BLOK ====";
$dblok=$this->db->query("select distinct(blok) as blok from data_peserta order by blok asc")->result();
foreach($dblok as $blok)
{
	$dbas[$blok->blok]=$blok->blok;
}
$dbas=$dbas;
 echo form_dropdown("blok",$dbas,"0",'class="form-control input-sm" id="blok" onchange="reload_table()"');
 ?>
 </div> 
 ...
 $dbas="";
$dbas["0"]="==== PIC ====";
$dblok=$this->db->query("select distinct(pic) as pic from data_peserta")->result();
foreach($dblok as $blok)
{
	$dbas[$blok->pic]=$blok->pic;
}
$dbas=$dbas;
 echo form_dropdown("pic",$dbas,"0",'class="form-control input-sm " data-live-search="true" id="pic" onchange="reload_table()"');
 ?>
 </div>
 ...
 $dbas="";
$dbas[""]="==== NO SURAT ====";
$dblok=$this->db->query("select distinct(CAST(no_surat AS SIGNED INTEGER)) as no_surat from data_peserta where  CAST(no_surat AS SIGNED INTEGER)!='0' order by no_surat asc")->result();
foreach($dblok as $blok)
{
	$dbas[$blok->no_surat]=$blok->no_surat;
}
$dbas=$dbas;
 echo form_dropdown("no_surat",$dbas,"0",'class="form-control input-sm " multiple data-live-search="true"  id="no_surat" onchange="reload_table()"');
 ?>
 </div>
 ...
 $dbas="";
$dbas[""]="==== NAMA FILE ====";
$dblok=$this->db->query("select distinct(nama_file) as nama_file from data_peserta order by id_peserta DESC")->result();
foreach($dblok as $blok)
{
	$dbas[$blok->nama_file]=$blok->nama_file;
}
$dbas=$dbas;
 echo form_dropdown("nama_file",$dbas,"0",'class="form-control input-sm "   data-live-search="true"  id="nama_file" onchange="reload_table()"');
 ?>
</div> 
 <div class="col-md-3">
 <?php
 $dbas="";
$dbas[""]="==== STATUS CETAK ====";
$dblok=$this->db->query("select distinct(cetak) as cetak from data_peserta")->result();
foreach($dblok as $blok)
{
	$dbas[$blok->cetak]=$blok->cetak;
}
$dbas=$dbas;
 echo form_dropdown("cetak",$dbas,"0",'class="form-control input-sm "     id="cetak" onchange="reload_table()"');
 ?>
</div> 
<div class="col-md-3">
 <?php
 $dbas="";
$dbas[0]="==== GATE ====";
$dblok=$this->db->query("select distinct(gate) as gate from data_peserta")->result();
foreach($dblok as $blok)
{
	$dbas[$blok->gate]=$blok->gate;
}
$dbas=$dbas;
 echo form_dropdown("gate",$dbas,"0",'class="form-control input-sm "     id="gate" onchange="reload_table()"');
 ?>
</div>
...
<th class='thead col-white'   width='5px'>
				...
				//};?>
				<th  class='thead'>BARCODE 1</th>
				<th  class='thead'>BARCODE 2</th>
			 
				
				<th  class='thead'>BLOK</th>
				 
				 
				 <th  class='thead'>PENANGGUNG JAWAB</th>
				 <th  class='thead' width="50px">STATUS1</th>
				 <th  class='thead' width="50px">STATUS2</th>
				 <th  class='thead' width="50px">REKAP</th>
				<th  class='thead'>No SURAT</th>
				<th  class='thead'>GERBANG</th>
...
function setKode(ke,id)
{			var url="<?php echo base_url();?>myevent/setKode";
			var kode=$("#kod"+ke+id).val();
			$.post(url,{ke:ke,id:id,kode:kode},function(data){
				  if(data=="false")
				  {
   ...
   window.open("<?php echo base_url()?>myevent/cetak_barcode/?id="+checkbox_value,"_blank");
	...
	{
		 $.post("<?php echo base_url();?>myevent/setRekap/",{id:id,set:set},function(){
 ...
 alertify.confirm("<center>  Hapus data terpilih ?</center>",function(){
  ...
  <script type="text/javascript">
    ...
    }function not2(id)
    {
        ...
        // ajax delete data to database
          $.ajax({
            url : "<?php echo base_url();?>myevent/not2/"+id,
            type: "POST",
            data: "JSON",
            success: function(data)
            {
	...
	function acc2(id)
    {
        ...
        // ajax delete data to database
          $.ajax({
            url : "<?php echo base_url();?>myevent/acc2/"+id+"/<?php echo $this->uri->segment(3);?>",
            type: "POST",
            data: "JSON",
            success: function(data)
            {
