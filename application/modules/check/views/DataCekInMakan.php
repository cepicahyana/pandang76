...
if($cek->num_rows())
{
	...
	$titleform="";
    		...
    		//	$isi="<a target='_blank' href='".base_url()."file_upload/form/".$dataDB->id_admin."_".$dataDB->id_event."/".$isiUpload."'>click to view</a>";
				...
				$isi.="<tr style='text-align:left'><td> Jumlah Pengambilan </td><td>:</td><td> ".$data->j_makan." </td></tr>";
				echo "</table>";
			}
	...
	if($data->status=="0")
	{
		echo "<div class='sadow'><h1><img width='100px' src='".base_url()."plug/img/warning.png'>  <font color='red'>".$id."</font>  BELUM TERFERIVIKASI</h1></div>";
	}else
	{
	...
	if($cek->num_rows()>0)
		{
			...
			$dp=$cek->row();
			$isisv=explode(",",$dp->makan);
			if(count($isisv)>1 and $id!="12345689")
			{
				echo "
				<center>
			...
			<center> 
				...
				$this->event->updateStatusMakanPeserta($id);
		echo "<center>	<div class='sadow' style='color:green'><h1>  WELCOME </h1></div>
				<font size='5px'> ".$isi."</font>  
			</center> 
				</div>";
			}
