...
if($cek->num_rows())
{
	...
	$titleform="";
    		...
    		//	$isi="<a target='_blank' href='".base_url()."file_upload/form/".$dataDB->id_admin."_".$dataDB->id_event."/".$isiUpload."'>click to view</a>";
				...
				$isi.="<tr style='text-align:left'><td> Ketentuan  </td><td>:</td><td> ".$data->j_pb." Kali Photoboth</td></tr>";
				echo "</table>";
			}
	...
	if($cek->num_rows()>0)
		{
			...
			if(count($isisv)>1)
			{
			   ...
			   SUDAH PERNAH MELAKUKAN PHOTOBOTH</h2></span> 
			...
			<center> 
				...
				$this->event->updateStatusPbPeserta($id);
					$this->event->updateStatusSvPeserta($id);
				echo "<center>	<div class='sadow' style='color:green'><h1>  JUMLAH PHOTOBOTH : ".$data->j_souvenir." KALI</h1> </div>
			 <font size='5px'> ".$isi."</font>";
			}
