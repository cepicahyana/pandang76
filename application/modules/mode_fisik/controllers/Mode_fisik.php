...
class Mode_fisik extends MY_Controller
{
	...
	{
		$this->load->view('temp_fisik/main', $data);
	}
	function CheckClass()
	{
...
<li  ><a target="_blank" href="' . base_url() . 'myevent/pb/' . $dataDB->id_event . '"  >PHOTOBOTH</a></li> 
	...
	function getPb($id)
	{
		...
		foreach ($dt as $va) {
			if ($va) {
				$tbl .= "<tr><td>" . $no . "</td><td>" . $this->tanggal->hariLengkap(substr($va, 0, 10), "/") . " " . substr($va, 11, 10) . "</td></tr>";
				$no++;
			}
		}
		$tbl .= "</table>";
